...
void AdaptiveModel::adaptiveTesselation() {
    ...
    for(int i = 0; i < modelPatches.size(); i += 1) {
        ...
        float u1[3] = {0, 0, 1};
        float v1[3] = {0, 1, 1};
        createTriangles(u1, v1, patch);
        float u2[3] = {0, 1, 1};
        float v2[3] = {0, 0, 1};
        createTriangles(u2, v2, patch);
    }
}
...
void AdaptiveModel::createTriangles(float u[], float v[],
    ...
    glm::vec3 a = bezpatchinterp(patch, u[0], v[0], n1);
    glm::vec3 b = bezpatchinterp(patch, u[1], v[1], n2);
    glm::vec3 c = bezpatchinterp(patch, u[2], v[2], n3);
    float abu = (u[0] + u[1])/2;
    float abv = (v[0] + v[1])/2;
    float bcu = (u[1] + u[2])/2;
    float bcv = (v[1] + v[2])/2;
    float acu = (u[0] + u[2])/2;
    float acv = (v[0] + v[2])/2;
    bool ab = isflat((a + b) / 2.0f, bezpatchinterp(patch,
        abu, abv, temp), errorBound);
    bool bc = isflat((b + c)/ 2.0f, bezpatchinterp(patch,
        bcu, bcv, temp), errorBound);
    bool ac = isflat((a + c)/ 2.0f, bezpatchinterp(patch,
    ...
    if (ab and bc and ac) {
        ...
        createTriangles(nextu1, nextv1, patch);
        float nextu2[3] = {u[2], acu, u[1]};
        float nextv2[3] = {v[2], acv, v[1]};
        createTriangles(nextu2, nextv2, patch);
    } else if (ab and !bc and ac) {
        ...
        float nextu1[3] = {u[0], (u[2] + u[3]) /2, u[2]};
        float nextv1[3] = {v[0], (v[2] + v[3]) /2, v[2]};
        createTriangles(nextu1, nextv1, patch);
        float nextu2[3] = {u[1], (u[2] + u[3]) /2, u[2]};
        float nextv2[3] = {v[1], (v[2] + v[3]) /2, v[2]};
        createTriangles(nextu2, nextv2, patch);
    } else if (ab and !bc and !ac) {
        ...
        createTriangles(nextu3, nextv3, patch);
    } else if (!ab and !bc and ac) {
        ...
        createTriangles(nextu4, nextv4, patch);
    }
